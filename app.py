...
def clean_number(val):
    """Məbləğləri təmizləyib düzgün float rəqəmə çevirir"""
    if pd.isna(val):
        return 0.0
...
if df_qaime is not None or df_odenis is not None:
    ...
    with col1:
        secilmis_musteri = st.selectbox("Müştərini (Qarşı Tərəfi) Seçin", musteriler_list if musteriler_list else ["Məlumat Tapılmadı"])
    with col2:
    ...
    if st.button("🚀 Avtomatik Üzləşmə Aktı Yarat"):
        ...
        if not combined_rows:
            ...
            # Başlıq Bölməsi (Sabit Şəki Şərab MMC + Seçilmiş Müştəri)
            st.markdown("---")
